...
同一个 Web 服务新增 <http://127.0.0.1:8765/monitor>，固定跟踪安洁科技（002635）、
依顿电子（603328）、威星智能（002849）。首页的“盘中监控”链接可直接进入。

```bash
.venv/bin/banxia-strategy serve --watch-date 2026-09-23
```

- 后台启动时立即采集，之后每60秒通过 mootdx 获取三只股票的盘口及一分钟线。
...
- 默认选取包含三只股票的最新日报，启动后固定使用这一份计划，不自动替换股票。
  计划交易日与当前日期不一致时停用入场判断；更新计划后可重新启动服务。
